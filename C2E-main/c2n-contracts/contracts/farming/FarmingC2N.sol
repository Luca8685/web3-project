...
contract FarmingC2N is Ownable {
    ...
    function pending(
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 timestampToCompare = pool.lastRewardTimestamp < endTimestamp
                ...
                : endTimestamp;
            //计算上次奖励时间到现在，过去了多少秒(时间段)
            uint256 nrOfSeconds = lastTimestamp - timestampToCompare;
            //计算这个LP在这段时间内，可以拿到的奖励
            uint256 erc20Reward = (nrOfSeconds *
                rewardPerSecond *
                pool.allocPoint) / totalAllocPoint;
            //(erc20Reward*1e36/lpSupply) = 这段时间拿到的奖励/总质押量 = 每个质押可以拿到的奖励
            //计算到现在 累积单位token质押奖励数
            accERC20PerShare =
                accERC20PerShare +
                ((erc20Reward * 1e36) / lpSupply);
        }
        //计算用户可以拿到的奖励 = 用户提供的LP的数量 * 累积单位token质押奖励数 - 用户之前用户质押总奖励数
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        erc20Transfer(msg.sender, pendingAmount);
        user.amount = user.amount - _amount;
        //这里虽然奖励全转出去了，但要用剩下的LP的数量 * 累积单位token质押奖励数，
        //来算出 用户质押总奖励数 用于计算下次的奖励
        user.rewardDebt = (user.amount * pool.accERC20PerShare) / 1e36;
        // 撤回流动性
