...
export default function WalletButton(props) {
  const { walletAddress, breContract, saleContract, getAccount, showWallet } =
    useWallet();
  ...
  return walletAddress ? (
    ...
    >
      ...
      ></IconWalletButton>{" "}
      {walletAddress && walletAddress.replace?.(/^(.{6}).+(.{4})$/g, "$1...$2")}
    </div>
  ) : (
    <div
      className={styles["wallet-button"] + " " + (props.className || "")}
      onClick={() => showWallet(true)}
    >
      <IconWalletButton
        style={{ verticalAlign: "text-bottom" }}
