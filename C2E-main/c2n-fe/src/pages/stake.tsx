...
export default function Pools() {
  const { chain, switchNetwork, validChains, isWalletInstalled } = useWallet();
  ...
  const tokenInfo = useMemo(() => {
    return (
      tokenSymbols.find((item) => item.chainId == chain?.chainId) ||
      tokenSymbols[0]
    );
  }, [chain]);
    ...
    <main className={styles["container"] + " container"}>
      ...
      <section className={styles["intro"] + " main-content"}>
        ...
        <div className={styles["info"]}>
          There is NO extra fee for {tokenInfo.symbol} token staking, NO extra
          fee for unlocking. Members in C2N are attracted by our promising
          projects, not restricted by us charging a fee. However, IDO projects
          might require their own different lock-up period, see more details
          from the
          ...
          <Row>
            <Col span={isDesktopOrLaptop ? 16 : 24}>
              Step1: Connect your wallet to stake {tokenInfo.symbol} token,
              which is required to participate in IDOs.
              <br />
              Step2: Use {tokenInfo.symbol} token to register for IDOs on C2N
              website, first come first serve.
              <br />
              Step3: Always remember, the longer you hold our BRE tokens, the
              more projects which have potentials and values you could
              participate with us!
              <br />
            </Col>
            <Col span={isDesktopOrLaptop ? 8 : 24}>
              <Row
                ...
                style={{
                  height: "100%",
                  marginTop: isDesktopOrLaptop ? 0 : "1em",
                }}
              >
                <BasicButton
                  className={styles["wallet-button"]}
                  onClick={() => {
                    addToken(tokenInfo.address, tokenInfo.symbol);
                  }}
                >
      ...
      <section className={styles["staking"]}>
        <div className="main-content">
          {/* 根据钱包安装状态和链连接状态显示不同内容 */}
          {
            ...
            ) : false && chain === undefined ? ( // 如果钱包已安装但链信息未定义，显示空内容
              <></>
            ) : true || !!chain ? ( // 如果钱包已安装且链信息有效，显示 StakingForm 组件
              <StakingForm available={true} poolId={0} />
            ) : (
              // 如果钱包已安装但链信息无效，显示网络错误信息和切换网络按钮
              <div style={{ padding: "2em 0", backgroundColor: "#ffffff" }}>
                ...
                <Row justify="center" align="middle">
                  <Col>
                    ...
                    <b>{validChains && validChains[0].name}</b>. Please switch
